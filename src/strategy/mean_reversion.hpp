...
namespace hft
{
    ...
    public:
        MeanReversion(StrategyContext& ctx,
                      RiskManager& risk,
                      spsc::Queue<EngineCommand, 1 << 14>& out,
                      std::size_t window_len = 64,
                      double dev_ticks = 2.0,
                      Qty quote_qty = 1)
            : ctx_(ctx), risk_(risk), out_(out),
              window_(window_len, 0), wlen_(window_len),
        ...
        void on_timer(u64 ts_ns) override
        {
            ...
            if (last_top_.bid_price == 0 || last_top_.ask_price == 0) return;
            ...
            if (!risk_.can_quote(quote_qty_)) return;
