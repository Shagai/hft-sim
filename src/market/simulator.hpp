        ...
        void step(Engine& engine)
        {
            ...
            else
            {
                ...
                if (make_spread_wider)
                {
                    ...
                    NewOrder b{next_order_id_++, street_user_, Side::Buy, best_bid - cfg_.tick, 5, TIF::Day, now_ns()};
                    NewOrder s{next_order_id_++, street_user_, Side::Sell, best_ask + cfg_.tick, 5, TIF::Day, now_ns()};
                    engine.inject_new(b);
                    engine.inject_new(s);
                }
                else
